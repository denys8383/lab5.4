...
double S0(const int K, const int N)
{
	double s = 0;
	for (int k = N; k <= 19; k++)
		s += sqrt((sin(1.0 *k) * sin(1.0 * k)) + (cos(N /(1.0 * k)) * cos(N / (1.0 * k))));
	return s;
}
double S1(const int K, const int N, const int i)
{
	if (i > 19)
		...
		return sqrt((sin(1.0 * i) * sin(1.0 * i)) + (cos(N / (1.0 * i)) * cos(N / (1.0 * i)))) + S1(K, N, i + 1);
}
double S2(const int K, const int N, const int i)
{
	if (i < N)
		...
		return sqrt((sin(1.0 * i) * sin(1.0 * i)) + (cos(N / (1.0 * i)) * cos(N / (1.0 * i)))) + S2(K, N, i - 1);
}
double S3(const int K, const int N, const int i, double t)
{
	t = t + sqrt((sin(1.0 * i) * sin(1.0 * i)) + (cos(N / (1.0 * i)) * cos(N / (1.0 * i))));
	if (i >= 19)
		return t;
	else
		return S3(K, N, i + 1, t);
